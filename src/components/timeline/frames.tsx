...
export function TimelineFrames(props: {
    ...
    function redrawTimelineCanvas(width?: number, height?: number) {
        ...
        let x = 0;
        const sampleDrawWidth = Math.ceil(props.model.gif.width * (canvasRect.height / props.model.gif.height));
        while (x < canvasRect.width) {
            const progress = x / canvasRect.width;
            const frame = props.model.getFrame(Math.floor(props.model.frameCount * progress));
            if (frame) {
...
function Ticks(props: {
    ...
    return (
        <>
            {props.model.gif?.frames.map((_, i) => (
